...
const invoicesDir = join(dirname(fileURLToPath(import.meta.url)), '../entries');

if (!existsSync(invoicesDir)) {
  mkdirSync(invoicesDir);
}

export const generateEntryPDF = async (incoming) => {
  ...
  try {
    ...
    doc.text(`Número de factura: ${incoming.entryNumber}`, rightX, 22, { align: 'right' });
    ...
    const tableRows = incoming.materialEntryDetail.map(
    ...
    doc.autoTable({
      ...
      didDrawPage: function (data) {
        ...
        doc.setFontSize(12);
        doc.setFont('bold');
        doc.text(`Número de factura: ${incoming.entryNumber}`, data.settings.margin.right, data.settings.margin.top - 2, { align: 'left' });
      }
    });
    ...
    const pdfPath = `${invoicesDir}/${incoming.entryNumber}.pdf`;
