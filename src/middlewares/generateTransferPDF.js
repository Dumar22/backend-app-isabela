...
const transferDir = join(dirname(fileURLToPath(import.meta.url)), '../transfers');

if (!existsSync(transferDir)) {
  mkdirSync(transferDir);
}

export const generateTransferPDF = async (incoming) => {
  ...
  try {
    ...
    doc.setFontSize(17);
    doc.setFont('bold');
    doc.text('Factura de Traslado', doc.internal.pageSize.getWidth() / 2, y, { align: 'center' });
    ...
    doc.text(`Número de factura: ${incoming.transferNumber}`, rightX, 22, { align: 'right' });
    doc.text(`Fecha: ${incoming.date}`, rightX, 22 + 5, { align: 'right' });
    doc.text(`Origen: ${incoming.origin}`, rightX, 22 + 10, { align: 'right' });
    doc.text(`Destino: ${incoming.destination}`, rightX, 22 + 15, { align: 'right' });
    doc.text(`Envia: ${incoming.delivery}`, rightX, 22 + 20, { align: 'right' });
    doc.text(`Recibe: ${incoming.receive}`, rightX, 22 + 25, { align: 'right' });
    doc.text(`Autoriza: ${incoming.autorization}`, rightX, 22 + 30, { align: 'right' });

    y += 10; // Ajustar la posición vertical del contenido
    
    // Agregar la tabla de materiales
    const tableHeader = [['Código', 'Nombre', 'Unidad', 'Estado', 'Cantidad', 'Valor unitario', 'Total']];
    const tableRows = incoming.materialTransferDetail.map(
    ...
    doc.autoTable({
      ...
      didDrawPage: function (data) {
        ...
        doc.setFontSize(12);
        doc.setFont('bold');
        doc.text(`Número de factura: ${incoming.transferNumber}`, data.settings.margin.right, data.settings.margin.top - 2, { align: 'left' });
      }
    });
    ...
    const pdfPath = `${transferDir}/${incoming.entryNumber}.pdf`;
